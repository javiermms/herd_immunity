...
class Simulation(object):
	...
	def _simulation_should_continue(self):
		...
		if self.dead == self.population_size or self.initial_infected == 0:
			return False
		else:
			return True
			...
			for person in infected_people:
				for interaction in range(100):
					...
					while random_person == None:
						person_picked = self.population[random.randint(0, len(self.population)-1)]
						if person_picked.is_alive:
							random_person = person_picked
					self.interaction(self.population[person], random_person)

			for person in infected_people:
				#hmmmmm
					person = self.population[person]
					survived = person.did_survive_infection(self.mortality_rate)
